...
fun Language.checkLanguage(oldLanguage: Language): Boolean {
    if (this.languageName != oldLanguage.languageName) {
        ...
        throw UtilsException(Resources.getSystem().getString(com.ludaxord.projectsup.R.string.utils_exception_change_to_same_language))
    }
}
...
fun View.setMonthForCalendarView(calendarView: AbstractSupCalendarView, newEvents: ArrayList<Date>?, amount: Int) {
    if (this is ImageView || this is Button) {
        ...
        throw UtilsException(Resources.getSystem().getString(com.ludaxord.projectsup.R.string.utils_exception_calendar_view_button))
    }
}
